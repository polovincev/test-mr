...
async def create_chat(data: ChatCreateIn, repo: ChatRepository = Depends(get_chat_repo)) -> Chat:
    ...
    if data.mode in ("goal", "profile_goal"):
        ...
        try:
            ...
            user_instruction = (
                "Начни диалог приветствием и предложением помочь сформулировать учебную цель, дальше веди диалог используя системный промпт, в приветствии добавляй имя Николай."
                + order_text
                if data.mode == "goal"
                else "Начни диалог приветствием и предложением рассказать о себе, дальше веди диалог используя системный промпт, в приветствии добавляй имя Николай."
                + order_text
            )
